...
pub enum TuiMessage {
    Visiting(String),
    Saved { url: String, filepath: String },
    AuthFail { url: String, code: u16 },
    Error { url: String, message: String },
    Progress { visited: usize, total: usize },
    Done { saved: usize, failed: usize, auth_failed: usize },
}
    ...
    loop {
        while let Ok(msg) = rx.try_recv() {
            match msg {
                ...
                TuiMessage::Done { saved, failed, auth_failed } => {
    ...
    loop {
        ...
        if let Some(ref mut rx) = active_rx {
            ...
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    TuiMessage::Visiting(url) => {
                        app.push_log(LogEntry { status: LogStatus::Visiting, url, filepath: None, message: None });
                    }
                    TuiMessage::Saved { url, filepath } => {
                        app.push_log(LogEntry { status: LogStatus::Ok, url, filepath: Some(filepath), message: None });
                    }
                    TuiMessage::AuthFail { url, code } => {
                        app.push_log(LogEntry { status: LogStatus::AuthFail(code), url, filepath: None, message: Some("auth failed, skipped".into()) });
                    }
                    TuiMessage::Error { url, message } => {
                        app.push_log(LogEntry { status: LogStatus::Error, url, filepath: None, message: Some(message) });
                    }
                    TuiMessage::Progress { visited, total } => {
                        app.set_progress(visited, total);
                    }
            ...
            if !done && active_handle.as_ref().map_or(false, |h| h.is_finished()) {
                active_rx = None;
                active_handle = None;
                stop_flag = None;
                                ...
                                match parse_command(&input) {
                                    Command::Scrape { url, max_pages, selector, prefix, delay, no_browser } => {
                                        if !url.starts_with("http://") && !url.starts_with("https://") {
                                            app.push_info("Invalid URL: must start with http:// or https://".into());
                                        } else {
                                            let effective_selector = selector.or_else(|| app.selector.clone());
                                            ...
                                            let use_browser = !no_browser && matches!(engine_kind, EngineKind::BrowserLazy);
                                            ...
                                            let handle = tokio::spawn(async move {
                                                let result: Result<(), color_eyre::Report> = async {
                                                    if use_browser {
                                                        let _ = tx.send(TuiMessage::Visiting("Launching browser...".into()));
                                                        if sf_clone.load(Ordering::Relaxed) {
                                                            return Err(color_eyre::eyre::eyre!("Stopped"));
                                                        }
                                                        let engine = Engine::new().await?;
                                                        engine.set_cookies(&cookies_clone).await?;
                                                        if sf_clone.load(Ordering::Relaxed) {
                                                            return Err(color_eyre::eyre::eyre!("Stopped"));
                                                        }
                                                        crawl(&engine, &crawl_cfg, Some(&*sf_clone), |ev| send_crawl_event(&tx, ev, &sf_clone)).await?;
                                                    } else {
                                                        let client = HttpClient::new(&cookies_clone)?;
                                                        crawl(&client, &crawl_cfg, Some(&*sf_clone), |ev| send_crawl_event(&tx, ev, &sf_clone)).await?;
                                                    }
                                                    Ok(())
                                                }.await;
                                                if let Err(e) = result {
                                                    let _ = tx.send(TuiMessage::Error { url: "engine".into(), message: e.to_string() });
                                                    let _ = tx.send(TuiMessage::Done { saved: 0, failed: 0, auth_failed: 0 });
                                                }
                                            });
                                    ...
                                    Command::Set { key, value } => {
                                        match key.as_str() {
                                            "output" => { app.output_dir = value; app.push_info("output updated.".into()); }
                                            "delay" => {
                                                if let Ok(d) = value.parse() { app.delay_ms = d; app.push_info("delay updated.".into()); }
                                                else { app.push_info("Invalid delay value.".into()); }
                                            }
                                            "max_pages" | "max-pages" => {
                                                if let Ok(m) = value.parse() { app.max_pages = m; app.push_info("max_pages updated.".into()); }
                                                else { app.push_info("Invalid max_pages value.".into()); }
                                            }
                                            "selector" => { app.selector = Some(value); app.push_info("selector updated.".into()); }
                                    ...
                                    Command::Cookies => {
                                        ...
                                        match cookies::extract_cookies(&browser_kind, None) {
                                            Ok(new_cookies) => {
                                                ...
                                                app.push_info(format!("{} cookies loaded.", app.cookie_count));
                                            }
                                            Err(e) => app.push_info(format!("Cookie reload failed: {}", e)),
                                        }
                                    }
                                    Command::Clear => { app.log.clear(); app.scroll_offset = 0; }
                                    Command::Help => {
                                        app.push_info("/scrape <url> [-m N] [-s sel] [-p pfx] [-d ms] [--no-browser]".into());
                                        app.push_info("/stop or [Esc] — stop current scrape".into());
                                    ...
                                    Command::Quit => break,
                                    Command::Unknown(msg) => { if !msg.is_empty() { app.push_info(msg); } }
                                }
                            }
                            _ => {}
                        }
...
fn send_crawl_event(tx: &UnboundedSender<TuiMessage>, ev: CrawlEvent, stop: &AtomicBool) {
    if stop.load(Ordering::Relaxed) && !matches!(ev, CrawlEvent::Done { .. } | CrawlEvent::Error(..)) {
        return;
    }
    let msg = match ev {
