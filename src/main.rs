    ...
    match cfg.url {
        Some(ref url) => {
            ...
            if !cfg.no_browser {
                ...
                if headless {
                    run_headless(&engine, &crawl_cfg).await?;
                } else {
                    run_with_tui(engine, &crawl_cfg, &cfg.browser, cookies.len()).await?;
                }
            } else {
                let client = HttpClient::new(&cookies)?;
                if headless {
                    run_headless(&client, &crawl_cfg).await?;
                } else {
                    run_with_tui(client, &crawl_cfg, &cfg.browser, cookies.len()).await?;
                }
            }
        }
        None => {
            ...
            if cfg.headless || !is_tty() {
                color_eyre::eyre::bail!(
                    "Interactive mode requires a terminal. Provide a URL for headless mode."
                );
            }
...
async fn run_headless<N: Navigate>(engine: &N, cfg: &CrawlConfig) -> Result<()> {
    ...
    crawl(engine, cfg, None, |event| match event {
        ...
        CrawlEvent::Done {
            ...
            pb.finish_and_clear();
            println!(
                "Done: {} saved, {} failed, {} auth errors",
                saved, failed, auth_failed
            );
        }
        _ => {}
    })
