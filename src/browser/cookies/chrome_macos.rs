...
fn decrypt_value(encrypted: &[u8], key: &[u8; KEY_LEN]) -> Result<String> {
    if encrypted.len() <= 3 { return Ok(String::new()); }
    ...
    let pt = dec.decrypt_padded::<Pkcs7>(&mut buf)
...
pub fn extract(profile: Option<&str>) -> Result<Vec<Cookie>> {
    ...
    let cookies: Vec<Cookie> = stmt.query_map([], |row| {
        ...
        Ok((name, encrypted, domain, path, expires_utc, is_secure, is_httponly, samesite))
    })?.filter_map(|r| r.ok()).filter_map(|(name, encrypted, domain, path, expires_utc, is_secure, is_httponly, samesite)| {
        ...
        Some(Cookie {
            ...
            domain,
            path,
            expires: if expires_utc <= 0 { -1.0 } else { (expires_utc / 1_000_000 - CHROME_EPOCH_OFFSET) as f64 },
            secure: is_secure != 0,
            http_only: is_httponly != 0,
            same_site: match samesite { 1 => SameSite::Lax, 2 => SameSite::Strict, _ => SameSite::None },
        })
    }).collect();
