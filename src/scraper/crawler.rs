...
const ASSET_EXTS: &[&str] = &["png","jpg","jpeg","gif","svg","css","js","woff","pdf","zip","ico","woff2","ttf","eot"];
...
pub enum CrawlEvent {
    ...
    AuthFail(String, u16),
    Error(String, String),
    Progress { visited: usize, saved: usize, queue: usize },
...
pub async fn crawl<N: Navigate>(engine: &N, config: &CrawlConfig, stop: Option<&AtomicBool>, mut on_event: impl FnMut(CrawlEvent)) -> Result<usize> {
    ...
    while let Some(url) = queue.pop_front() {
        ...
        if saved >= config.max_pages { break; }
        if visited.contains(&url) { continue; }
        visited.insert(url.clone());

        let u = match Url::parse(&url) { Ok(u) => u, Err(_) => continue };
        if u.origin().ascii_serialization() != origin { continue; }
        if let Some(ext) = u.path().rsplit('.').next() {
            if ASSET_EXTS.contains(&ext.to_lowercase().as_str()) { continue; }
        }
        if let Some(ref pfx) = config.prefix {
            if !u.path().starts_with(pfx.as_str()) { continue; }
        }
        ...
        for link in &result.links {
            if let Ok(lu) = Url::parse(link) {
                ...
                if lu.origin().ascii_serialization() == origin && !visited.contains(&s) {
                    if let Some(ref pfx) = config.prefix {
                        if !lu.path().starts_with(pfx.as_str()) { continue; }
                    }
                    queue.push_back(s);
                }
            }
        ...
        match html_to_markdown(&result.html, config.selector.as_deref()) {
            Ok(page) => {
                ...
                let title = if !result.title.is_empty() { &result.title } else { &page.title };
                match write_page(&config.output_dir, &fp, title, &url, &page.markdown) {
                    Ok(_) => { saved += 1; on_event(CrawlEvent::Saved(url.clone(), fp)); }
                    Err(e) => { failed += 1; on_event(CrawlEvent::Error(url.clone(), e.to_string())); }
                }
            }
            Err(e) => { failed += 1; on_event(CrawlEvent::Error(url.clone(), e.to_string())); }
        }
        ...
        if visited.len() % 20 == 0 {
            on_event(CrawlEvent::Progress { visited: visited.len(), saved, queue: queue.len() });
        }
    ...
    on_event(CrawlEvent::Done { saved, failed, auth_failed });
    Ok(saved)
}
